...
class TaskTest extends WebTestCase {
    ...
    public function test_Task_IsEqual() {
        $task1 = new Task('componentName');
        $task2 = new Task('componentName');
        $this->assertTrue($task1->isEqual($task2));
        $this->assertTrue($task1->isEqual($task2));

        $task1->setPriority(1);
        $this->assertFalse($task1->isEqual($task2));
        $this->assertFalse($task1->isEqual($task2));
        ...
        $task1->add($child1 = new Task('child1'));
        $this->assertFalse($task1->isEqual($task2));
        $this->assertFalse($task1->isEqual($task2));

        $task2->add($child1 = new Task('child1'));
        $this->assertTrue($task1->isEqual($task2));
        $this->assertTrue($task1->isEqual($task2));

        $task1->add($child2 = new Task('child2'));
        $this->assertFalse($task1->isEqual($task2));
        $this->assertFalse($task1->isEqual($task2));
    ...
    public function test_TaskRoot_isEqual() {
        $root1 = new TaskRoot('root');
        $root2 = new TaskRoot('root');

        $this->assertTrue($root1->isEqual($root2));
        $this->assertTrue($root1->isEqual($root2));

        $root1->setTimeBox(100);		

        $this->assertFalse($root1->isEqual($root2));
        $this->assertFalse($root1->isEqual($root2));
